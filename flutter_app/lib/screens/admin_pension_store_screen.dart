...
class AdminPensionStoreScreen extends StatefulWidget {
  const AdminPensionStoreScreen({super.key});

  @override
  State<AdminPensionStoreScreen> createState() =>
      _AdminPensionStoreScreenState();
}

class _AdminPensionStoreScreenState extends State<AdminPensionStoreScreen> {
  bool _isLoading = true;
  List<PensionWinningNumber> _rounds = [];
  final Map<int, int> _loadedStoreCount = {};
  final Map<int, List<LotteryStore>> _loadedStores = {};
  ...
  Future<void> _loadStores(PensionWinningNumber item) async {
    setState(() => _loadingSet.add(item.round));

    // Supabase에서 조회만 (로컬 저장X)
    ...
    setState(() {
      ...
      _loadedStores[item.round] = stores;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _rounds.isEmpty
              ? const Center(child: Text('발행된 연금복권 회차가 없습니다'))
              : ListView(
  ...
  Future<void> _publish(int round) async {
    final stores = _loadedStores[round];
    if (stores == null || stores.isEmpty) {
      _showSnack('먼저 지점로드를 해주세요');
      return;
    }

    setState(() => _publishingSet.add(round));

    // Supabase에 발행 상태 저장 → 홈에서 조회 가능
    await SupabaseService.publishPensionStores(round);
    ...
    _showSnack('$round회차 발행완료! (${stores.length}건)');
  }

  Widget _buildRoundCard(PensionWinningNumber item) {
