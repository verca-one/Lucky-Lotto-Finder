...
class DHLotteryCrawler:
    ...
    def crawl_speedlotto_stores(self, game_type: str, round_num: int) -> bool:
        ...
        try:
            ...
            logger.error(f"스피또 {game_type} 크롤링 실패: {e}")
            return False

    def save_to_json(self, game_type: str, filename: str) -> bool:
        """데이터를 JSON 파일로 저장"""
        try:
            filepath = f"zero_plus_base_{game_type}_stores_latest.json"
            ...
            logger.info(f"{game_type} 데이터 저장 완료: {filepath}")
            return True
        except Exception as e:
    ...
    def run(self):
        """전체 크롤링 실행"""
        logger.info("=" * 50)
        logger.info("동행복권 당첨지점 크롤링 시작 (전회차)")
        logger.info("=" * 50)
