...
PENSION_INFO_URL = "https://www.dhlottery.co.kr/game/gameResult.do"
...
def get_latest_round():
    ...
    try:
        # 최신 회차 페이지 접속
        url = "https://www.dhlottery.co.kr/game/gameResultFive.do"
        ...
        import re
        matches = re.findall(r'(\d{1,3})회', text)

        if matches:
            # 가장 큰 숫자를 찾기 (최신 회차)
            latest_round = max(int(m) for m in matches if int(m) <= 1000)  # 1000회 이상은 제외
            print(f"🔍 최신 연금복권 회차 감지: {latest_round}회")
            return latest_round
...
def get_stored_latest_round(db):
    ...
    try:
        ...
        query = stores_ref.where('lottery_type', '==', 'pension').order_by('round', direction=firestore.Query.DESCENDING).limit(1)
            ...
            for row in rows[1:]:
                ...
                if len(cols) >= 3:
                    ...
                    if store_name:
                        ...
                        store_id = f"pension_{region}_{store_name}_{address.split()[0]}"
...
def save_to_firebase(db, round_num, stores):
    ...
    try:
        ...
        for store in stores:
            doc_id = f"pension_{round_num}_{store['store_name']}"
...
def main():
    """메인 크롤링 함수 (전 회차 수집)"""
    print("💰 연금복권 당첨지점 크롤링 (전 회차 베이스 자료 구축)")
    print("=" * 70)
    ...
    # 회차 범위 설정
    START_ROUND = 1
    END_ROUND = 312  # 최신 회차를 여기에 입력해주세요
    ...
    for round_num in range(END_ROUND, START_ROUND - 1, -1):
        print(f"📊 [{END_ROUND - round_num + 1}/{END_ROUND}] 연금복권 {round_num}회 크롤링 중...", end=" ")
    ...
    # 최종 JSON 저장
    if all_stores:
        output_file = "base_pension_stores_latest.json"
