...
SPEETO2000_INFO_URL = "https://www.dhlottery.co.kr/game/gameResultSpeeto.do"
...
def fetch_speeto2000_stores(round_num):
    """스피또2000 당첨지점 크롤링"""
    try:
        url = f"{BASE_URL}?method=viewResult&drwNo={round_num}&gameName=SPT2"
        headers = {
            ...
            for row in rows[1:]:
                ...
                if len(cols) >= 3:
                    ...
                    if store_name:
                        ...
                        store_id = f"speeto2000_{region}_{store_name}_{address.split()[0]}"
        ...
        print(f"❌ 스피또2000 {round_num}회 크롤링 실패: {e}")
        return None
...
def save_to_firebase(db, round_num, stores):
    ...
    try:
        ...
        for store in stores:
            doc_id = f"speeto2000_{round_num}_{store['store_name']}"
            doc_ref = db.collection('stores').document(doc_id)
            batch.set(doc_ref, {
                "lottery_type": "speeto2000",
                "round": round_num,
                "rank": store['rank'],
...
def main():
    """메인 크롤링 함수 (전 회차 수집)"""
    print("✨ 스피또2000 당첨지점 크롤링 (전 회차 베이스 자료 구축)")
    print("=" * 70)
    ...
    for round_num in range(START_ROUND, END_ROUND + 1):
        print(f"📊 [{round_num}/{END_ROUND}] 스피또2000 {round_num}회 크롤링 중...", end=" ")

        stores = fetch_speeto2000_stores(round_num)
    ...
    # 최종 JSON 저장
    if all_stores:
        output_file = "base_speeto2000_stores_latest.json"
