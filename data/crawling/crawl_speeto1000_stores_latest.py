...
def fetch_speeto1000_stores(round_num):
    """스피또1000 당첨지점 크롤링"""
    try:
        url = f"{BASE_URL}?method=viewResult&drwNo={round_num}&gameName=SPT1"
        headers = {
            ...
            for row in rows[1:]:
                ...
                if len(cols) >= 3:
                    ...
                    if store_name:
                        ...
                        store_id = f"speeto1000_{region}_{store_name}_{address.split()[0]}"
        ...
        print(f"❌ 스피또1000 {round_num}회 크롤링 실패: {e}")
        return None
...
def save_to_firebase(db, round_num, stores):
    ...
    try:
        ...
        for store in stores:
            doc_id = f"speeto1000_{round_num}_{store['store_name']}"
            doc_ref = db.collection('stores').document(doc_id)
            batch.set(doc_ref, {
                "lottery_type": "speeto1000",
                "round": round_num,
                "rank": store['rank'],
...
def main():
    """메인 크롤링 함수 (전 회차 수집)"""
    print("✨ 스피또1000 당첨지점 크롤링 (전 회차 베이스 자료 구축)")
    print("=" * 70)
    ...
    for round_num in range(END_ROUND, START_ROUND - 1, -1):
        print(f"📊 [{END_ROUND - round_num + 1}/{END_ROUND}] 스피또1000 {round_num}회 크롤링 중...", end=" ")

        stores = fetch_speeto1000_stores(round_num)
    ...
    if all_stores:
        output_file = "base_speeto1000_stores_latest.json"
