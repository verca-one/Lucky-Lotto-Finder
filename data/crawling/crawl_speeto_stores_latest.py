...
SPEETO_INFO_URL = "https://www.dhlottery.co.kr/game/gameResultSpeeto.do"
...
def get_latest_round():
    """최신 스피또 회차 감지"""
    try:
        response = requests.get(SPEETO_INFO_URL, timeout=10)
        ...
        if matches:
            ...
            print(f"🔍 최신 스피또 회차 감지: {latest_round}회")
            return latest_round
...
def get_stored_latest_round(db):
    ...
    try:
        ...
        query = stores_ref.where('lottery_type', '==', 'speeto').order_by('round', direction=firestore.Query.DESCENDING).limit(1)
...
def fetch_speeto_stores(round_num):
    """스피또 당첨지점 크롤링"""
    try:
        url = f"{BASE_URL}?method=viewResult&drwNo={round_num}&gameName=SPT"
        headers = {
            ...
            for row in rows[1:]:
                ...
                if len(cols) >= 3:
                    ...
                    if store_name:
                        ...
                        store_id = f"speeto_{region}_{store_name}_{address.split()[0]}"
        ...
        print(f"❌ 스피또 {round_num}회 크롤링 실패: {e}")
        return None
...
def save_to_firebase(db, round_num, stores):
    ...
    try:
        ...
        for store in stores:
            doc_id = f"speeto_{round_num}_{store['store_name']}"
...
def main():
    """메인 크롤링 함수 (전 회차 수집)"""
    print("✨ 스피또 당첨지점 크롤링 (전 회차 베이스 자료 구축)")
    print("=" * 70)
    ...
    # 회차 범위 설정
    START_ROUND = 1
    END_ROUND = 250  # 최신 회차를 여기에 입력해주세요
    ...
    for round_num in range(START_ROUND, END_ROUND + 1):
        print(f"📊 [{round_num}/{END_ROUND}] 스피또 {round_num}회 크롤링 중...", end=" ")

        stores = fetch_speeto_stores(round_num)
    ...
    # 최종 JSON 저장
    if all_stores:
        output_file = "base_speeto_stores_latest.json"
